...
def test_notebooks_exist(material_dir):
    ...
    expected_subdirs = {"Pyomo Fundamentals", "Nonlinear Exercises", "Dynamic Exercises", "GDP Exercises"}
    assert len(subdirs_with_notebooks & expected_subdirs) > 0, "No notebooks found in expected subdirectories"
...
def test_sample_notebook_execution(material_dir):
    ...
    fundamental_notebooks = [
        ...
        if "Pyomo Fundamentals" in str(nb) and "1.1" in str(nb)
    ]
...
def test_notebooks_have_proper_structure(material_dir):
    ...
    for notebook_path in notebooks:
        ...
        assert 'markdown' in cell_types, f"Notebook {notebook_path} has no markdown cells"
        ...
        for i, cell in enumerate(nb.cells):
            if cell.cell_type == 'code':
                ...
                if 'input(' in source:
                    pytest.fail(f"Notebook {notebook_path} cell {i} contains input() which will hang in automated execution")
                
                # Check for absolute paths that might be problematic
                if any(path in source for path in ['/Users/', '/home/', 'C:\\', 'D:\\']):
                    # This is a warning, not a failure, as some paths might be examples
                    print(f"Warning: Notebook {notebook_path} cell {i} contains absolute paths")
